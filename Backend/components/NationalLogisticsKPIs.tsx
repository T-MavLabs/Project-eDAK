...
export function NationalLogisticsKPIs({ metrics }: { metrics: SummaryMetrics }) {
  ...
  const avgTransitDays = metrics.avgNationalTransitDays ?? 0;
  const totalRoutes = metrics.totalActiveRoutes ?? 0;
  const criticalHub = metrics.criticalHub ?? { name: "N/A", code: "N/A", congestionScore: 0 };
  const lowestMode = metrics.lowestReliabilityMode ?? { mode: "N/A" as const, reliabilityScore: 0 };
  ...
  const kpis = [
    {
      ...
      icon: Route,
      isRisk: avgTransitDays > 3.0,
    },
    {
      label: "Total Active Routes",
      value: totalRoutes.toLocaleString("en-IN"),
      icon: MapPin,
      isRisk: false,
    },
    {
      label: "Critical Hub (Most Congested)",
      value: `${criticalHub.name} (${criticalHub.code})`,
      icon: AlertTriangle,
      isRisk: criticalHub.congestionScore > 7.0,
    },
    {
      label: "Lowest Reliability Mode",
      value: `${lowestMode.mode} (${(lowestMode.reliabilityScore * 100).toFixed(0)}%)`,
      icon: TrendingDown,
      isRisk: lowestMode.reliabilityScore < 0.75,
    },
  ];
    ...
    <div>
      <div className="mb-6">
        ...
        <div className="text-sm text-muted-foreground leading-relaxed max-w-3xl">
          ...
          red indicate risk thresholds requiring operational attention.
        </div>
      </div>
