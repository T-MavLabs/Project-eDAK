    ...
    <div className="grid gap-3 md:grid-cols-4">
      {items.map((it, idx) => {
        ...
        return (
          <Card 
            key={it.label} 
            className="border-primary/10 daksh-glass daksh-elevated daksh-transition daksh-fade-in"
            style={{ animationDelay: `${idx * 50}ms` }}
          >
            <CardHeader className="space-y-1">
              <CardTitle className="flex items-center justify-between text-sm font-medium text-muted-foreground">
                {it.label}
                <Icon className="h-4 w-4 text-muted-foreground daksh-transition" />
              </CardTitle>
            </CardHeader>
            <CardContent>
              <div className={it.valueClassName ? `text-2xl font-semibold daksh-code ${it.valueClassName}` : "text-2xl font-semibold daksh-code"}>
                {it.value}
              </div>
            </CardContent>
          </Card>
